 ...
 *
 * An instance of this C++ functor can be passed to a number of asynchronous
 * Kerberos ticket creation functions to inform if ticket creation succeeded
 * or failed.
 * @see GDKerberosAuthHandler
 */

class GDKerberosAuthCallback
{
public:
    virtual ~GDKerberosAuthCallback() {}
    
    /**
     * Implement this function to be supplied a <tt>Krb5ErrorCode</tt> value
     * of <tt>KDC_ERR_NONE</tt> representing success or a different
     * <tt>Krb5ErrorCode</tt> value representing the reason for failure.
     */
 ...
 * Kerberos delegation work in the whole end-to-end application.
    ...
    /**
     * Asynchronously get a Kerberos ticket by supplying user name, password,
     * and whether it can be delegated.
     *
     * Call this function to asynchronously create an initial Kerberos ticket for
     * authentication. This will be a Ticket-to-Get-Tickets (TGT), for a
     * specified user principal. If the ticket is created, it will be stored in
     * the BlackBerry Dynamics secure cache.
     *
     ...
     *                        can be delegated.
     *
     * @param callback <tt>GDKerberosAuthCallback&</tt> if ticket creation succeeded,
     *                 or a different <tt>Krb5ErrorCode</tt> value representing the
     *                 reason for failure.
     */
    ...
    /**
     * Asynchronously get a Kerberos ticket with a user name and password.
     *
     * Calling this function is equivalent to calling the following.\code
    ...
    /**
     * Asynchronously get a Kerberos ticket via PKINIT authentication.
     *
     * Call this function to asynchronously create an initial Kerberos ticket for
     * authentication. This will be a Ticket-to-Get-Tickets (TGT), for an
     * implicit user principal authenticated by using public key cryptography
    ...
    /**
     * Asynchronously get a Kerberos ticket via Kerberos Constrained Delegation,
     * with implicit credentials, and specify whether it can be delegated.
     *
     * Call this function to asynchronously create a Kerberos ticket for authentication.
     * This will be a service ticket obtained by Kerberos Constrained Delegation
     * (KCD) to a specified authentication host, with implicit credentials. If
     * the ticket is created, it will be stored in the BlackBerry Dynamics
     * secure cache.
     *
    ...
    /**
     * Asynchronously get a Kerberos ticket via delegation, with implicit credentials.
     *
     * Calling this function is equivalent to calling the following.\code
     * setUpKerberosTicketAsync(host, port, getAllowDelegation(), callback)
     * \endcode
     *
     * @param host <tt>char*</tt> pointer to memory containing the FQDN of the
     *             authentication server, and a null terminator.
